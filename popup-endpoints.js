...
/**
 * Test one Monitor and turn the outcome into a message for the user.
...
export async function testEndpoint(url, providerId) {
    ...
    // One reader for both paths: this is the same call the poller makes.
    const outcome = await readMonitor({ name: 'Test connection', url }, provider);

    if (outcome.ok) {
        if (!outcome.hasCount) {
            Logger.error('Monitor test error: response had no count');
    ...
    Logger.error(`Monitor test error: ${outcome.status}`);
    return { success: false, message: describeReadFailure(outcome, url) };
}

/** User-facing copy for a failed Monitor read. */
function describeReadFailure(outcome, url) {
    let domain = url;
    try {
        domain = new URL(url).hostname;
    } catch {
        // Unparseable URL — show it verbatim.
    }

    switch (outcome.status) {
        case 'unauthenticated':
            // The reader distinguishes "no credentials configured" from
            // "credentials rejected", and its message is already user-facing.
            return outcome.errorType === 'missingCredentials'
                ? outcome.message
                : `Not logged in to ${domain} — open ${domain} in this browser and sign in, then try again`;
        case 'rate-limited':
            return outcome.retryAfter
                ? `Rate limited — try again in ${outcome.retryAfter}s`
                : 'Rate limited — try again shortly';
        case 'timed-out':
            return `Timed out after ${REQUEST_TIMEOUT_MS / 1000} seconds waiting for ${domain}`;
        case 'malformed':
            return 'Invalid API response format';
        case 'http-error':
            return `HTTP ${outcome.httpStatus}`;
        default:
            return outcome.message || 'Failed to connect to monitor';
    }
}
